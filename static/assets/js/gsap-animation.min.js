...
function pbmit_img_animation() {
  ...
  gsap.utils
    .toArray(".pbmit-animation-style7 .elementor-widget-container")
...
function pbmit_hover_img() {
  const pbmitHoverImg = gsap.utils.toArray(
    ".pbmit-ihbox-style-2,.pbmit-timeline .pbmit-content, .pbmit-team-style-2 .pbminfotech-box-content-inner"
  );
  pbmitHoverImg.forEach((target) => {
    ...
    target.pbmitHoverAnim = t1.play().reversed(!0);
...
function pbmit_horizontal_snap_section() {
  ...
  let pbmithsContainer = document.querySelector(
      ".pbmit-horizontal-snap-section"
    ),
    pbmithsPanel = gsap.utils.toArray(
      ".pbmit-horizontal-snap-section .pbmit-ele-horizontal-snap"
    ),
    pbmithWidth = 0,
    pbmithsTween;
  ScrollTrigger.matchMedia({
          ...
          scrollTrigger: {
            ...
            start: () =>
              ...
              (section.offsetLeft - window.innerWidth / 2) *
                (pbmithWidth / (pbmithWidth - window.innerWidth)),
            end: () =>
              "+=" +
              section.offsetWidth *
                (pbmithWidth / (pbmithWidth - window.innerWidth)),
    ...
    "(max-width: 1200px)": function () {
      if (pbmithsTween) pbmithsTween.kill(!0);
...
function pbmit_staticbox_hover() {
  var pbmit_var = jQuery(
    ".pbmit-element-static-box-style-1, .pbmit-element-static-box-style-2"
  );
  if (!pbmit_var.length) {
    return;
  }
  pbmit_var.each(function () {
    var pbmit_Class =
      ".pbmit-element-posts-wrapper > .pbmit-ele-static-box, .swiper-static-slide-nav li, .pbmit-hover-inner li";
...
function pbmit_title_animation() {
  ...
  const quotes = document.querySelectorAll(
    ".pbmit-heading-subheading .pbmit-element-title, .pbmit-custom-heading .pbmit-element-title "
  );
  quotes.forEach((quote) => {
    ...
    var getclass = quote.closest(
      ".pbmit-heading-subheading, .pbmit-custom-heading"
    ).className;
      ...
      let tl = gsap.timeline({
        scrollTrigger: {
          ...
          trigger: pbmit_sticky_container,
            ...
            "+=" +
            (section.height() + 200 - document.documentElement.clientHeight),
        },
...
function pbmit_sticky_menu() {
  ...
  ScrollTrigger.matchMedia({
    "(min-width: 992px)": function () {
      ...
      var indicators = gsap.utils.toArray(
        ".pbmit-element-split-slide-style-1 .pbmit-split-slide-item-image"
      );
      var points = gsap.utils.toArray(
      ...
      let tl = gsap.timeline({
        scrollTrigger: {
          ...
          trigger: container,
            ...
            "+=" +
            (section.height() + gap - document.documentElement.clientHeight),
          onUpdate: (self) => {
      ...
      jQuery(
        ".pbmit-element-split-slide-style-1 .pbmit-bg-imgbox .pin-spacer .pbmit-split-slide-item-image:first-child"
      ).addClass("active");
    },
    "(max-width:992px)": function () {
      ScrollTrigger.getAll().forEach((pin) => pin.kill(!0));
    },
  });
}
function pbmit_bg_change() {
  ...
  $startTriggers.forEach((elm) => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        gsap.to($section, { backgroundColor: color, overwrite: "auto" });
